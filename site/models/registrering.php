...
//$model =  new RegnModelRegistrering;
...
class RegnModelRegistrering extends ListModel
{
    ...
    function oppdater_saldo($ar)
    {
        ...
        $db = Factory::getDbo();
        $sql = $db->getQuery(true);
        $sql = 'select buntnr from qo7sn_regn_hist WHERE bilagsart=0 AND Dato="' . $ar . '01-01";';
        $db->setQuery((string) $sql);
        try {
            $buntnr = $db->loadAssoc();
        } catch (Exception $e) {
    ...
    function sistepost()
    {
        ...
        $db = $this->getDbo();
        $query = $db->getQuery(true);
        $query = 'select * FROM ( SELECT * from ' . $database . 'qo7sn_regn_trans WHERE Buntnr!=0 AND Bilagsart!=0 order by ref DESC  LIMIT 29) AS reversed  ORDER BY ref;';
        // $query = 'select * from ' . $database . 'qo7sn_regn_trans WHERE Buntnr!=0 AND Bilagsart!=0 order by ref   LIMIT 29;';
        $db->setQuery((string) $query);
        $message1 = $db->loadObjectList();
        $svar = json_encode(end($message1));
            $sv = [];
        if (!$message1) {
            $sv["Dato"] = date("Y-m-d");
            $sv["debet"] = "";
    ...
    function transer($regnskapsar)
    {
        ...
        $db = $this->getDbo();
        $query = $db->getQuery(true);
        $query = 'select * FROM ( SELECT * from ' . $database . 'qo7sn_regn_trans WHERE Buntnr!=0 AND Bilagsart!=0 order by ref DESC  LIMIT 29) AS sub ORDER BY ref ASC ;';
        $db->setQuery((string) $query);
        try {
