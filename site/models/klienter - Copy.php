...
*/
    ...
    function oppdater_firma($firma)
    {
        ...
        if ($value) {
            $query = 'update ' . $value . '.qo7sn_regn_firma';
        } else {
            $query = 'update #__regn_firma ';
        }

        $query = $query .
            " set Firmanavn = " . $db->quote($firma->Firmanavn) . ",
            Adresse = " . $db->quote($firma->Adresse) . ",
            Postnr = " . $db->quote($firma->Postnr) . ",
            Poststed = " . $db->quote($firma->Poststed) . ",
            kontaktperson = " . $db->quote($firma->kontaktperson) . ",
            telefon = " . $db->quote($firma->telefon) . ",
            epost = " . $db->quote($firma->epost) . ",
            debitorkto = " . $db->quote($firma->debitorkto) . ",
            kreditorkto = " . $db->quote($firma->kreditorkto) . ",
            neste_bilagsnr = " . $db->quote($firma->neste_bilagsnr) . ",
            buntnr = " . $db->quote($firma->buntnr) . ",
            periode = " . $db->quote($firma->periode) . ",
            bilagref = " . $db->quote($firma->bilagref) . ",
      
            antall_perioder = " . $db->quote($firma->antall_perioder) . ",
            regnskapsar = " . $db->quote($firma->regnskapsar) . ",
            regnskapsmodus = " . $db->quote($firma->regnskapsmodus) . ",
            Skannet_lagringsmappe = " . $db->quote($firma->Skannet_lagringsmappe) . ",
            valgt_periode = " . $db->quote($firma->valgt_periode) . ",

            passord = " . $db->quote($firma->passord) . ",
            brukernavn = " . $db->quote($firma->brukernavn) . ";";
        ...
        */
        ...
        try {
            ...
            if ($ret)
                ...
                $sql = 'insert into klienter (ID,fornavn,etternavn,firma,brukernavn,telefon,epost,folder_name) values (1,"test","test","test","reg100001","123456780","test@test.com","reg100001")';
                $db->setQuery((string) $sql);
                try {
                    $db->execute(); // Returns
                    $array = ["idref" => "reg100001", "ID" => 1, "firma" => "test", "brukernavn" => "reg100001", "telefon" => "123456780", "epost" => "test@test.com", "passord" => "test"];
    ...
    function opprett_klient($array)
    {
        ...
        $letters = preg_replace('/[^a-zA-Z]/', '', $array->folder_name); // "abcdef"

        // Extract only numbers
        $numbers = (int) preg_replace('/[^0-9]/', '', $array->folder_name); // "123456"
        $numbers++;
        $nyfolder = $letters . $numbers;
        ...
        //       $sql = insert into klienter (ID,fornavn,etternavn,firma,brukernavn,telefon,epost)  '.$array['ID'].';';  
        $sql = 'insert into klienter (idref,firma,folder_name,brukernavn,telefon,epost,passord) '
            . 'values ("' . $array->idref . '","' . $array->firma . '","' . $array->folder_name . '","' . $array->brukernavn . '","' . $array->telefon . '","' . $array->epost . '","' . $array->passord . '");';
        $db->setQuery((string) $sql);
        try {
            $db->execute(); // Returns 
        } catch (Exception $e) {
