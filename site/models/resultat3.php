...
class RegnModelResultat extends ListModel
{
    ...
    function startvariable()
    {
     ...
     //   $sql = 'select * from #__regn_hist order by ref desc limit 1;';
        ...
        $query = $db->getQuery(true);
        if ($value > '') {
            $query = 'select *  from ' . $value . '.#__regn_hist order by Buntnr desc limit 1;';
        } else {
 ...
 //       $query = 'select * from #__regn_trans order by Buntnr desc limit 1;';
    ...
    public function saldoliste($mode, $ar, $periode)
    {
        ...
        IF ($value > '') {
            $query = 'select *  from ' . $value . '.ROM qo7sn_regn_saldo WHERE ar="' . $ar
                . '"  and periode="' . $periode . '"  order by raplinje;';
        } else {
            $query = 'select *  from ROM qo7sn_regn_saldo WHERE ar="' . $ar
                . '"  and periode="' . $periode . '"  order by raplinje;';
        }
    ...
    public function historikk($ar)
    {
        ...
        $query = $db->getQuery(true);
        IF ($value > '') {
            $query = 'select *  from ' . $value . '.qo7sn_regn_hist WHERE YEAR(Dato)=' . $ar;
        } else {
    ...
    public function regnskapsar()
    {
        ...
        $query = $db->getQuery(true);
        IF ($value > '') {
            $query = 'select *  from ' . $value . '.qo7sn_regn_regnskapsar order by regnskapsar desc';
        } else {
    ...
    public function perioder($mode)
    {
       ...
       $db = $this->getDbo();
        ...
        if ($mode == '0')
            $query = ' SELECT * FROM ' . $value . '.qo7sn_regn_perioder order by nr ';
        else if ($mode == '1')
            $query = ' SELECT * FROM ' . $value . '.qo7sn_regn_perioder order by Periodenavn ';
        else
            $query = ' SELECT * FROM ' . $value . '.qo7sn_regn_perioder where Periodenavn="' . $mode . '";';
        }
        ...
        else
{
    ...
    public function hentkto()
    {
        ...
        $query = $db->getQuery(true);
        IF ($value > '') {
            $query = 'select *  from ' . $value . '.#__regn_kto order by Ktonr;';
        } else {
    ...
    public function getListQuery1($h)
    {
        ...
        IF ($value > '') {
            $query = 'select *  from ' . $value . '.#__regn_hist where belop=' . $h . ' limit 10;';
        } else {
