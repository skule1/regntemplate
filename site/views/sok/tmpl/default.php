...
$username = $user->username;
if ($username)
	echo '<h3>Klient: ' . $user->username . '<br></h3>';
?>
<?php
...
$sql = 'select Bilagsart,Bilag,Dato,debet,kredit,belop,Tekst,kontoinfo from ' . $database . 'qo7sn_regn_hist where debet=4010;'; // and regnskapsar=2022 and Periode="februar" order by dato;';
$db->setQuery((string) $sql);
$messages = $db->loadObjectList();


$sql = 'select * from ' . $database . 'qo7sn_regn_kto order by Ktonr;';
$db->setQuery((string) $sql);
$messages1 = $db->loadObjectList();


$sql = 'select * from ' . $database . 'qo7sn_regn_perioder order by nr;';
$db->setQuery((string) $sql);
$messages2 = $db->loadObjectList();

$sql = 'select * from ' . $database . 'qo7sn_regn_regnskapsar order by regnskapsar desc;';
$db->setQuery((string) $sql);
...
<table>
	<tr>
		...
		<td>
			<input type="text" id="id_sok2" placeholder='Enter text' onkeypress="subm('id_sok2')"
				onchange="oppdat_sok('id_sok2')">
		</td>
		<td>
...
<script type="text/javascript">
	...
	async function oppdat_sok(ii) {
		...
		if (valg1 == 'dato') {
			...
			toggleInput('id_sok1', 'hidden', '');
			toggleInput('id_sok2', 'text', 'Oppgi bunt');
			toggleInput('id_sok3', 'hidden', '');
			document.getElementById("id_htmlsok").innerHTML = ' ';
			document.getElementById("id_sok2").focus();
			...
			success: function (response) {
				...
				//if ((valg1 == 'sql') || (valg1 == 'kto'))
				{
					...
					if (valg1 == 'kto') {
						...
						console.log('bel1', formbel1, ' bel2', formbel2, ' diff', formdiff);
