...
// $model2 = $this->getModel('klienter');
// $model3 = $this->getModel('Resultat');

// $perioder3 = $model2->startvariable();
//$firma = $model->firma();
...
//$database = $modela->hent_database($username);
// if ($database[0] == "X")
...
//$regnskapsarliste = $regnskapsarliste["regnskapsar"];
...
$oppstartvar = $model->startvariable($regnskapsar);
...
<form action="" method="GET" style=" margin-left: 20px;">
    ...
    if (!$transer) {
        ...
        //  echo 'resp: '.$res.'<br>';
        ?>
        <select id="valg_ar" Name="valg_ar" onchange="endre_ar()">

            <?php

            foreach ($regnskapsarliste as $regnskapsar1) {

                // echo $regnskapsar1 . '<br>';
                if ($regnskapsar1["regnskapsar"] == $regnskapsar)
                    echo '<option value=' . $regnskapsar1["regnskapsar"] . ' selected>' . $regnskapsar1["regnskapsar"] . '</option>';
                else
                    echo '<option value=' . $regnskapsar1["regnskapsar"] . '>' . $regnskapsar1["regnskapsar"] . '</option>';
            }
        ...
        echo $regnskapsar; ?>
    Modus: <select id="modus" style="width:100px;" modus" Size="Number_of_options" onchange="f_modus()"
        value="<?php echo $modus ?>">
...
<form action="" method="get" name="reg">
    ...
    /* Her behandles hver post i transaksjopnsregisteret,kalt $message */
...
if ($result = mysqli_query($link, $sql)) {
       ...
       */
            if ($modus == 'valuta') {
                if ($message->currency_amount == 0)
                    ...
                    $msg = $msg . '<td  width="200"; > ' . number_format($message->currency_amount, 2, ',', '.') . ' ' . $message->currency . "</td>";
            }
            ...
            $msg = $msg
                ...
                . '<td  > <input type="text" id="kontoinfo' . $message->Ref . '"  style="width:100px;padding-left: 10px; border-width:0px;" onclick="btn(' . $message->Ref . ')" onchange="updatefield(' . $message->Ref . ')"  value="' . substr($message->kontoinfo, 0, 80) . '"></td>'
    ...
    <td align="right" style="text-align:right;width:50px; border-width:0px;  padding-right: 10px;">
        <?php echo ($ref + 1) ?>
    </td>
    ...
    //  echo '<td  style="text-align:right;"> <input type="text" id="bilagsart' . $message->Ref . '"  onclick="btn(' . $message->Ref . ')"  onchange="updatefield(' . $message->Ref . ')" style=" width:30px; border-width:0px; direction: rtl;" value="' . $message->Bilagsart . '"></td>'
    //     . '<td  style="text-align:right;"> <input type="text" id="dato' . $message->Ref . '"  onclick="btn(' . $message->Ref . ')"  onchange="updatefield(' . $message->Ref . ')" style=" width:115px; border-width:0px; direction: rtl;"  value="' . $message->Dato . '"></td>'
    //     . '<td  style="text-align:right;"> <input type="text" id="debet' . $message->Ref . '"   onclick="btn(' . $message->Ref . ')" onchange="updatefield(' . $message->Ref . ')" style=" width:50px; border-width:0px; direction: rtl;"   value="' . $message->debet . '"></td>'
    //     . '<td  style="text-align:right;"> <input type="text" id="kredit' . $message->Ref . '"  onclick="btn(' . $message->Ref . ')"  onchange="updatefield(' . $message->Ref . ')" style=" width:50px; border-width:0px; direction: rtl;"  value="' . $message->kredit . '"></td>';
    
    ?>
    <td><input class="fff" type="text1" id=1 style=" width:30px;" onkeydown="list_art()" onclick="list_art()"
            onchange="list_art()"> </td>


    <!--td><input type="text" id=1 style=" width:30px;" onkeydown="finn_kto()"   > </td-->
    <td><input class="fff" type="text1" placeholder="DD.MM.YYYY" name="Dato" id=2 style=" width:115px;"
            onchange="datokonv4(<?php echo $regnskapsar ?>,0)" onclick="datokonv4(<?php echo $regnskapsar ?>,1)"> </td>

    <td><input class="fff" type="text1" id=3 name="debet" style=" width:50px;" onclick="finn_kto(0)"
            onkeydown="finn_kto(1)" onchange="finn_kto(2)">
    </td>

    <td><input class="fff" type="text1" id=4 style=" width:50px;" onclick="finn_kkto(0)" onkeydown="finn_kkto(1)"> </td>

    <td><input class="fff" type="text1" id=5 style=" width:150px;" onkeydown="belop('<?php echo $modus ?>')">
    </td>

    <td><input class="fff" type="text1" id=6 style=" width:200px;"
    ...
    <td align="right"><input type="hidden" style=" width:150px;  border-width:0px; text-align:left;" id="ref"
...
<script type="text/javascript">
    ...
    function endre_ar() {
        ...
        jQuery.ajax({
            type: "POST",
            url: "index.php?option=com_regn&task=registrering.endre_ar",
            data: ({
                ar: ar
            }),
            cache: false,
    ...
    async function UpdateRecord(ref, bilagsnr, buntnr, mode) {
        ...
        console.log("envent key: " + event.key, ref, bilagsnr, buntnr, mode);

        let vv = "";

        // if (event.key !== 'Enter') {
        //     document.getElementById(id_tekst).value = "";
        //     vv="";
        //     console.log("UpdateRecord vv etter ikke enter: |" + vv + "| ref: " + ref);
        // } else
        vv = document.getElementById(6).value;
        const bk = vv;
        console.log("UpdateRecord vv før: |" + vv + "|");
        if (vv === undefined)
            return;
        console.log("UpdateRecord vv: |" + vv + "| ref: " + ref, bk);
        let sistepost = <?php echo $sistepost ?>;

        if ((vv == ' ') || ((vv == '') && (event.key == ' '))) {
            vv = sistepost['tekst'].trim();
            document.getElementById(id_tekst).value = vv;
        ...
        var belop = document.getElementById(5).value;
        // if (mode == 'valuta')
        var valuta = 0;// document.getElementById("val").value;
        //    else var valuta = 0;
    ...
    */
    ...
    async function finn_kto(i) { //i=0: onclick, i=1: onchange
        ...
        let sistepost = <?php echo json_decode($sistepost) ?>;
        console.log('sistepost', sistepost);
        console.log('sistepost dato', sistepost["Dato"]);
        ...
        if (((kto == '0') || (kto == ' ')) && (kto.length == 1)) {
            ...
            console.log('kto: |', kto, '|siste post: ', sistepost);
        }
        ...
        //    if (event.key == 'Enter')
        {
            ...
            jQuery.ajax({
                ...
                success: function (tekst) {
                    //                        console.log('retur: |', tekst, '|');
                    let gg =
                        '<table  style=" margin-left: 20px; width:400px;" border="0"  cellspacing="0" cellpadding="0"><tr><th style="text-align:right; width:150px; padding: 0px 10px 0px 0px;" >Debet</th><th style="text-align:left; width:100px; ">Navn</th></tr><tr>';
                    //    console.log(gg);
                    if (tekst) {
                        ...
                        if (isJsonArray(tekst)) {
                            ...
                            for (let i = 0; i < obj2.length; i++) {
                                gg = gg +
                                    '<tr> <td style="text-align:right; border-width:0px; " >  <input type="text" style="text-align:right; border-width:0px;" ' +
                                    'id="' + i + 'oo" value="' + obj2[i].Ktonr + '"  onclick="hent_kto(' + obj2[i].Ktonr + ',' + i + ')"></td>' +
                                    ' <td style="text-align:left; border-width:0px; " ><input type="text"  style="text-align:left; border-width:0px;" ' +
    ...
    async function finn_kkto(arg) { //i=0: onclick, i=1: onchange
        ...
        jQuery.ajax({
            ...
            success: function (tekst) {
                ...
                //   '<table  style=" margin-left: 20px;" border="0" cellspacing="0" cellpadding="0"><tr><th style="text-align:right; width:400px; padding: 0px 10px 0px 0px;" >Kredit</th><th style="text-align:left; width:100px; ">Navn</th></tr><tr>';
                ...
                if (isJsonArray(tekst)) {
                    ...
                    for (let i = 0; i < obj2.length; i++) {
                        ...
                        gg = gg + ' <tr> <td style="text-align:right; border-width:0px; " >  <input type="text" style="text-align:right; border-width:0px;" ' +
                            'id="oo" value="' + obj2[i].Ktonr + '"  onclick="hent_kkto(' + obj2[i].Ktonr + ',' + i + ')"></td>' +
                            ' <td style="text-align:left; border-width:0px; " ><input type="text"  style="text-align:left; border-width:0px;" ' +
    ...
    function datokonv4(regnskapsar, mode) {
        //    document.getElementById(3).focus;
        console.log('datokonv4', regnskapsar, mode);
        ...
        console.log('debet:', debet, ' kredit:', kredit, ' dato:', dato, ' belop:', belop  );
        if (mode == 0)
            if (event.key == 'Enter') {
                if (!document.getElementById(id_debet).value)
                    document.getElementById(id_debet).focus();
                else if (!document.getElementById(id_kredit).value)
                    document.getElementById(id_kredit).focus();
                else if (!document.getElementById(id_belop).value)
                    document.getElementById(id_belop).focus();
                return;
            }
        ...
        if (((dato == '0') || (dato == ' ')) && (dato.length == 1)) {
            dato = sistepost['Dato'];
            document.getElementById(3).value = dato;
            console.log('dato: |', dato, '|siste post: ', sistepost);
        }
        ...
        //     today = '1.1.2020';
        console.log('datokonv4 today: ', today);
        ...
        // let firma_regnskapsar = "<?php echo $regnskapsar ?>";
        let forrigedato = sistepost["Dato"];
        console.log('sistepost', sistepost);
        ...
        if (forrigedato == null) {
            ...
            fdag = forrigedato.substring(8, 10);
            fmnd = forrigedato.substring(5, 7);
            far = forrigedato.substring(0, 4);
            console.log('fdag: ', fdag, ' fmnd: ', fmnd, ' far: ', far);
        }

        //const obj1 = JSON.parse(forrigepost);
        //  console.log(obj1);
        dato = document.getElementById(id_dato).value;
        if ((dato == ' ') || (dato == '0')) dato = fdag + ',' + fmnd + ',' + far;
        //           if ((dato == ' ') || (dato == '0')) dato = fdag + '.' + fmnd + '.' + far;
        //         if ((dato == ' ') || (dato == '0')) dato = far + ',' + fmnd + ',' + fdag;
        // debet = document.getElementById(id_debet).value;
        // kredit = document.getElementById(id_kredit).value;
        //   let belop = document.getElementById(id_belop).value;
        belop = 0;
        ...
        if (debet != '') hent_kto(debet, 4);
        if (kredit != '') hent_kkto(kredit, 4);
    ...
    function list_art1(i) {
        ...
        jQuery.ajax({
            ...
            // url: "/components/com_regn/views/Registrering/tmpl/update.php",
            type: "POST",
            url: "index.php?option=com_regn&task=registrering.listart",
            //     url: "/components/com_regn/views/Bilagsart/tmpl/update.php",
            cache: false,
            success: function (tekst) {
                //      console.log(tekst);
        ...
        if (art == '')
            ...
            $.ajax({
                ...
                success: function (tekst) {
                    ...
                    // //      document.getElementById("5").focus();
                    if ((obj2.dato == null) || (obj2.dato == '0000-00-00')) document.getElementById(id_dato).focus();
