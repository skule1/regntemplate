...
if (isset($_POST['mode']) && $_POST['mode'] == "artcheck")
	f_artcheck();
elseif (isset($_POST['mode']) && $_POST['mode'] == "slett")
	f_slett();
elseif (isset($_POST['mode']) && $_POST['mode'] == "debet_oppdat_sok")
	f_debet_oppdat_sok();
elseif (isset($_POST['mode']) && $_POST['mode'] == "debet_oppdat_bev")
	f_debet_oppdat_bev();
elseif (isset($_POST['mode']) && $_POST['mode'] == "kredit_oppdat_bev")
	f_kredit_oppdat_bev();
...
function f_slett()
{
	...
	// global $hash, $conn;
	// $id = $_POST['id'];
	// $sql = 'DELETE FROM ' . $hash . 'regn_bilagsart WHERE id='.$id.';';
	// echo 'sql: '.$sql.'<br>';
...
function f_debet_oppdat_sok()
{
	...
	$sok = $_POST['sok'];
	$valg = $_POST['valg'];
	//echo "f_debet_oppdat_sok '.$valg.'<br>";
	//echo 'f_debet_oppdat_sok  valg: ' . $valg . ' per1: ' . $per . ' ar1: ' . $ar . ' sok1: ' . $sok.'<br>';
	if ($valg == 'fritekst') {

		if ($ar == "Alle år" && $per == "Alle perioder")
			$sql = 'select sum(belop) as ss,count(belop) as cnt from ' . $hash . 'regn_hist where (tekst LIKE "%' . $sok . '%" or kontoinfo LIKE "%' . $sok . '%") ';
		elseif ($per == "Alle perioder")
			$sql = 'select sum(belop) as ss ,count(belop) as cnt from ' . $hash . 'regn_hist where  (tekst LIKE "%' . $sok . '%" or kontoinfo LIKE "%' . $sok . '%") and Regnskapsar=' . $ar;
		elseif ($ar == "Alle år")
			$sql = 'select sum(belop) as ss ,count(belop) as cnt  from ' . $hash . 'regn_hist where  (tekst LIKE "%' . $sok . '%" or kontoinfo LIKE "%' . $sok . '%")  and Periode="' . $per . '"';
		else
			$sql = 'select sum(belop) as ss ,count(belop) as cnt  from ' . $hash . 'regn_hist where  (tekst LIKE "%' . $sok . '%" or kontoinfo LIKE "%' . $sok . '%") and Periode="' . $per . '" and Regnskapsar=' . $ar;
		$sql = $sql . ';';
		// echo $sql;
		// if ($sort == "Dato")
...
function f_oppdat_sok()
{
	...
	if ($valg == 'fritekst') {
		...
		$sql = 'select * from #__regn_hist where debet=' . $sok . ' or kredit=' . $sok . ';';
		//echo 'beløpsøk: '.$sql.'<br>';
	}
...
function f_oppdat_bev()
{
	...
	$sqld = 'select sum(belop) from ' . $hash . 'regn_hist where (debet=' . $kto . ') and Periode="' . $per . '" and Regnskapsar=' . $ar;
	$sqlk = 'select sum(belop) from ' . $hash . 'regn_hist where (kredit=' . $kto . ') and Periode="' . $per . '" and Regnskapsar=' . $ar;
...
function perioder_oppdat()
{
	...
	if ($valg == "fjor") {
		...
		$sql = 'UPDATE qo7sn_regn_saldo SET budsjett=fjorar WHERE ar=' . $ar  . ' AND kto=' . $kto . ';';
		$result = $conn->query($sql);
		//    if ($result) echo 'opdatert '.$sql.'<br>';
		// for ($i = 1; $i < 13; $i++) {
