...
<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
  <select id="dropdown" name="mode" onchange="selectOption()">
    <option>Resulat</option>
    <option>Balanse</option>
    <option>Likvid</option>
  </select>
...
$query
  ...
  ->from('#__regn_kto')
  ->setLimit('20');
$query = 'select * from #__regn_kto where ResBal="' . $mode . '" order by Ktonr;';
...
<form action="" method="get" name="reg">
  <table id="e" border="0" cellspacing="1" cellpadding="1">
    ...
    $query = 'select * from qo7sn_regn_resmal where BR="' . $mode . '" order by nr;';
      ...
      foreach ($messages as $message) { ?>
        <tr>
          <td align="right"><input type="text"
              onmousedown="logg('<?php echo $message->Ktonr ?>','<?php echo $regnskapsar ?>')"
              name="<?php echo $message->Ktonr ?>" id="<?php echo $message->Ktonr ?>" style="width:50px; border-width:0px;"
              value="<?php echo $message->Ktonr ?>" </td>
          <td align="left" style="padding: 0px 0px 0px 10px;"><input type="text"
              onchange="oppdater_kto('<?php echo $message->Ktonr; ?>')" id="2<?php echo $message->Ktonr ?>" name=<?php echo $message->Navn ?>" style="width:300px; border-width:0px;" value="<?php echo $message->Navn ?>"></td>
          <td class="dropdown">
            <select style=" border-width:0px; background-color:white" name="nr" id="3<?php echo $message->Ktonr ?>"
              onchange="oppdater_kto('<?php echo $message->Ktonr; ?>')" value="'<?php echo $message->rapportlinje ?>')">
              <?php if ($messages1)
                foreach ($messages1 as $message1) {
                  echo ' <option value="' . $message1->nr . '"';
                  if ($message1->nr == $message->rapportlinje)
                    echo ' selected';  //and $messages1->BR==$message->ResBal
                  echo '>' . $message1->nr . ' ' . $message1->tekst . '</option>';
                }
              ?>;
              </select')">
          </td>
...
<table class="table1" width="1000" cellspacing="0" cellpadding="0" border="0">
  <tr>
    <td style="vertical-align:top" colspan="5">
      ...
      <br />
            ...
            <tr>
              ...
              <td><input type="text" name="prosjekt" id="prosjekt" online="none" value="<?php echo $prosjekt ?>" /></td>
            </tr>
            <tr>
...
<script type="text/javascript">
  ...
  function hent_perioder(ar1, kto1) {
    var i = 1;
    //    console.log("hent_perioder " + ar1 + '  ' + kto1);
    ...
    jQuery.ajax({
      ...
      success: function (tekst) {
       //    console.log('hent_perioder: ' + tekst);
        ...
        //    console.log('obj:  ' + obj[2].belop + " " + obj[2].ar);
...
*/
       ...
       //   + parseFloat(parseFloat(fjorarsum).toFixed(0)).toLocaleString("nb-NO") + ' id="fjor"></td></tr>';
        ...
        gg = gg + '<tr><th scope="row">Fordel på\nperioder: </th><td>'
          ...
          + '</tr>';
        gg = gg + '</table></div>';
        document.getElementById("kr2").innerHTML = gg;
